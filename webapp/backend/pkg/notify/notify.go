...
func (n *Notify) SendWebhookNotification(webhookUrl string) error {
	...
	if resp.StatusCode >= 400 {
		return fmt.Errorf("webhook to %s returned HTTP %d", webhookUrl, resp.StatusCode)
	}
	return nil
}
...
func (n *Notify) SendShoutrrrNotification(shoutrrrUrl string) error {
	...
	if err != nil {
		n.Logger.Errorf("An error occurred while generating notification payload for %s (%s): %v", serviceName, shoutrrrUrl, err)
		return err
	}
